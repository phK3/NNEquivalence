...
# modified from https://github.com/sisl/NNet/blob/master/examples/example_nnet2pb.py
class NNet():
    """
    ...
    def to_keras(self):
        ...
        !!! only for ReLU right now, no BatchNorm !!!

        :return:
        """
        in_layer = keras.layers.Input(shape=(self.inputSize,))

        for i in range(self.numLayers - 1):
            if i == 0:
                l = in_layer
            l = keras.layers.Dense(self.layerSizes[i + 1], activation='relu')(l)

        l = keras.layers.Dense(self.layerSizes[-1], activation='linear')(l)
        model = keras.models.Model(inputs=in_layer, outputs=l)

        for i in range(self.numLayers):
            model.layers[i + 1].set_weights([self.weights[i].T, self.biases[i]])

        return model

    def round_nnet_16bit(self):
        """
        Rounds this NN's weights to 16bit
        ...
        new_weights = []
        new_biases = []
        for k, weightMatrix in enumerate(self.weights):
            matrix = []
            for row in weightMatrix:
                matrix.append(list(map(lambda f: np.float16(f), row)))
            new_weights.append(np.array(matrix))

            bias = list(map(lambda f: np.float16(f), self.biases[k]))
            new_biases.append(np.array(bias))
